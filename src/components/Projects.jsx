  ...
  return (
    <section id="projects" className="py-24 px-4 max-w-7xl mx-auto overflow-hidden">
      ...
      <div className="relative flex flex-col items-center">
        ...
        {/* Pista de tarjetas con altura definida para dar cabida al proyecto de mayor tamaño */}
          ...
          {projectsData.map((project, index) => {
            ...
            return (
              <motion.article
                ...
                /* 
                   - h-[380px] sm:h-[400px]: Altura uniforme para todas las tarjetas.
                   - flex flex-col justify-between: Separa el contenido superior del inferior.
                */
                className={`absolute w-[310px] sm:w-[360px] h-[390px] sm:h-[400px] bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl p-6 sm:p-7 flex flex-col justify-between shadow-xl transition-colors duration-300 select-none ${
                  isCenter ? 'cursor-default border-[var(--badge-text)]/40 shadow-2xl' : 'cursor-pointer hover:border-[var(--badge-text)]'
                }`}
              >
                ...
                <div className="flex-1 flex flex-col">
                  <div className="flex justify-between items-start mb-3 gap-2">
                    ...
                    {project.featured && (
                      <span className="text-[10px] uppercase font-bold tracking-wider px-2.5 py-1 bg-[#D03B13] text-white rounded-md shadow-xs shrink-0">
                        Destacado
                      </span>
                    )}
                  </div>
                ...
                <div className="mt-auto pt-4">
                  <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-5">
                    {project.tags.map((tag, idx) => (
                      <span 
                        key={idx} 
                        className="inline-flex items-center text-[11px] font-semibold px-2.5 py-1 bg-[var(--badge-bg)] text-[var(--badge-text)] rounded-lg border border-[var(--badge-border)]"
                      >
                        <img 
        ...
        {/* Controles de Navegación y Dots */}
        <div className="flex items-center justify-between w-full max-w-xs mt-6 z-40">
          <button
            onClick={handlePrev}
            aria-label="Proyecto anterior"
            className="w-10 h-10 rounded-full border border-[var(--border-color)] bg-[var(--bg-card)] text-[var(--text-main)] flex items-center justify-center hover:border-[#D03B13] hover:text-[#D03B13] transition-colors shadow-xs active:scale-95"
          >
            ←
          </button>
            ...
            {projectsData.map((_, idx) => (
              <button
                ...
                className={`h-2.5 rounded-full transition-all duration-300 ${
                  currentIndex === idx 
                    ? 'w-8 bg-[#D03B13]' 
                    : 'w-2.5 bg-[var(--border-color)] hover:bg-[var(--text-muted)]'
                }`}
              />
            ))}
          </div>
